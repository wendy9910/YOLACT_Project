...
def postprocess(det_output, w, h, batch_idx=0, interpolation_mode='bilinear',
    ...
    boxes   = dets['box']
    scores  = dets['score']
    masks   = dets['mask'] #mask.shape(torch.Size([幾個obj, w, h, 遮罩]))

    if cfg.mask_type == mask_type.lincomb and cfg.eval_mask_branch:
        # At this points masks is only the coefficients  此時掩碼只是係數
        ...
        if cfg.use_maskiou:
            with timer.env('maskiou_net'):                
                with torch.no_grad():
                    maskiou_p = net.maskiou_net(masks.unsqueeze(1)) #unsqueeze(1)用於增加一個維度:...這個維度是給遮罩(:猜
