...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    """
    Note: If undo_transform=False then im_h and im_w are allowed to be None. img_w & img_h被允許為空值
    """
    if undo_transform:#啥意思
        img_numpy = undo_image_transformation(img, w, h) #原圖
        img_gpu = torch.Tensor(img_numpy).cuda()
    else:
        img_gpu = img / 255.0 #轉成(0~1)
        h, w, _ = img.shape
    ...
    if args.display_masks and cfg.eval_mask_branch and num_dets_to_consider > 0:
        ...
        # masks ...shape ... torch.Size([3, 640, 427, 1]) 、 type:torch.Tensor
        # mask.shape(torch.Size([幾個obj, w, h, 遮罩]))
        ...
        # masks_color = masks..(1*obj, 1*w, 1*h, 3*1) * colors * mask_alpha

        # colors = torch.Size([obj, 1, 1, 3])
        # masks_color = torch.Size([obj, 640, 427, 3])

        # This is 1 everywhere except for 1-mask_alpha where the mask is
        # 除了掩碼所在的 1-mask_alpha 外，其他地方都是 1
        # 找出遮罩以外的位置.....代表位置==0的
        # mask如果是0*mask_alpha=0 ...mask[]=0的位置是要去背的部分
        inv_alph_masks = masks * (-mask_alpha) + 1
        #結果:背景..0*-0.5+1=1 、 Mask..1*-0.5+1=0.5
        print(masks.shape)#torch.Size([3, 640, 427, 1])
        print(colors.shape)#torch.Size([3, 1, 1, 3])
        print(masks_color.shape)#torch.Size([3, 640, 427, 3])
        ...
        if num_dets_to_consider > 1:
            inv_alph_cumul = inv_alph_masks[:(num_dets_to_consider-1)].cumprod(dim=0) #dim=0时，第一行不变，后面每行将前面行对应元素乘起来
            masks_color_cumul = masks_color[1:] * inv_alph_cumul
            masks_color_summand += masks_color_cumul.sum(dim=0)

            print(inv_alph_cumul.shape)#torch.Size([2, 640, 427, 1])
            print(masks_color_cumul.shape)#torch.Size([2, 640, 427, 3])
            print(masks_color_summand.shape)#torch.Size([640, 427, 3])

        #最後合成 *(0 or 1)   img_gpu 原圖  inv_alph_masks.prod(dim=0)  masks_color_summand色塊
        #img_gpu inv_alph_masks masks_color_summand shape相同
        mask2 = masks.sum(dim=0)
        one = torch.ones_like(mask2)
        mask2 = torch.where(mask2 > 1.0,one,mask2)

       #將image通道改為四維
        # img_h,img_w,img_c=img_gpu.shape
        # new_img = img_gpu.resize_(img_h, img_w, 4)
        # new_img[:,:,-1]=255.0/255.0
        #print(new_img)

        # n = torch.FloatTensor(img_h, img_w, 4).fill_(255)
        # print(n)
        
        # print(new_img.shape)

        #img_gpu = img_gpu * inv_alph_masks.prod(dim=0) + masks_color_summand
        img_gpu = img_gpu * mask2
        print(inv_alph_masks.shape)#torch.Size([3, 640, 427, 1])
        print(img_gpu.shape)#torch.Size([640, 427, 3])

        #mask3 = torch.zero_like()

    print(masks.shape)
    print(masks[0][300][200][0])
    print(inv_alph_masks[0][0][0][0])
    ...
    img_numpy = (img_gpu * 255).byte().cpu().numpy() #乘回255....像數
...
class Detections:
    ...
    def dump_web(self):
        ...
        # 這些應該已經按照 prep_metrics 的工作方式按分數排序。
...
def prep_metrics(ap_data, dets, img, gt, gt_masks, h, w, num_crowd, image_id, detections:Detections=None):
    ...
    #返回此圖像的 AP 列表，每個元素對應一個類
    if not args.output_coco_json: #沒有在COCODataset中
...
def evalimage(net:Yolact, path:str, save_path:str=None):
    ...
    frame2 = torch.from_numpy(cv2.imread(path)).cuda().float()
    batch = FastBaseTransform()(frame2.unsqueeze(0))
...
def evalimages(net:Yolact, input_folder:str, output_folder:str):
    ...
    for p in Path(input_folder).glob('*'): 
        ...
        evalimage(net, path, out_path)#輸入net:網路架構、path:輸入照片的路徑、out_path:輸出照片的路徑
        print(path + ' -> ' + out_path)
    print('Done.')
...
def evaluate(net:Yolact, dataset, train_mode=False):
    ...
    if args.image is not None:
        if ':' in args.image:
            inp, out = args.image.split(':') #切割、(images=test_images:output_images)input & output
            evalimage(net, inp, out)
        else:
            evalimage(net, args.image)
        return
    elif args.images is not None:
        inp, out = args.images.split(':') #切割、(images=test_images:output_images)input & output
...
if __name__ == '__main__':
    ...
    if args.trained_model == 'interrupt': #--trained_model=./yolact/weights/yolact_plus_resnet50_54_800000.pth
        args.trained_model = SavePath.get_interrupt('weights/')
    elif args.trained_model == 'latest':
        args.trained_model = SavePath.get_latest('weights/', cfg.name)

    if args.config is None: #config=yolact_plus_resnet50_config
    ...
    if args.detect:  #score_threshold=0.15???
        cfg.eval_mask_branch = False

    if args.dataset is not None: #top_k=15 一張照片最多偵測15個物件
    ...
    with torch.no_grad():
        ...
        if args.image is None and args.video is None and args.images is None: #(images=test_images:output_images)input & output
